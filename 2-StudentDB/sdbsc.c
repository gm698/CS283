...
int del_student(int fd, int id)
{
    ...
    offset = (id - MIN_STD_ID) * sizeof(student_t);
    ...
    bytes_written = write(fd, &EMPTY_STUDENT_RECORD, sizeof(student_t));
...
int count_db_records(int fd)
{
    ...
    // If no records were found, handle the empty database case
    if (record_count == 0) {
        printf(M_DB_EMPTY);  // Or handle it as appropriate
    } else {
